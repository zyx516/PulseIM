...
public class MessageController {
    ...
    @PostMapping @ResponseStatus(HttpStatus.CREATED) public ApiResponse<MessageView> send(@RequestHeader("Authorization") String authorization, @RequestHeader(value = "X-Trace-Id", required = false) String traceId, @Valid @RequestBody SendMessageCommand command) { return ApiResponse.ok(TraceIds.from(traceId), messageService.persist(BearerTokens.require(authorization).userId(), authorization, command)); }
    ...
    @GetMapping("/dashboard") public ApiResponse<DashboardView> dashboard(@RequestHeader("Authorization") String authorization, @RequestHeader(value = "X-Trace-Id", required = false) String traceId) { String userId = BearerTokens.require(authorization).userId(); return ApiResponse.ok(TraceIds.from(traceId), messageService.dashboard(userId)); }
    ...
    @PostMapping("/{messageId}/ack") public ApiResponse<Void> acknowledge(@RequestHeader("Authorization") String authorization, @RequestHeader(value = "X-Trace-Id", required = false) String traceId, @PathVariable String messageId, @RequestBody(required = false) AckCommand command) { messageService.acknowledge(BearerTokens.require(authorization).userId(), authorization, messageId, command == null ? "ACK" : command.stage()); return ApiResponse.ok(TraceIds.from(traceId), null); }
    public record SendMessageCommand(@NotBlank String clientMessageId, @NotBlank String conversationId, @NotBlank String toUserId, @NotBlank @Size(max = 4000) String content, String replyToMessageId, @Size(max = 160) String replyPreview, @Size(max = 1000) String mentions) { }
